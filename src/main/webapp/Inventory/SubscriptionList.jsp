...
<%-- 							<strong><a href="<%=request.getContextPath()%>/RwaSubscriptionReport?type=getDetails"><%=Information.RwaSubscriptionType%></a></strong>--%>
					...
					<div class="col-sm-12 offset-sm-0 table-responsive">
						<table id="datatable" class="hover display nowrap dataTable table-condensed dtr-inline collapsed"
						 style="width: 100%">
...
<%-- 								<tr >
								...
								<th rowspan="2"><%=Information.RwaSubscriptionType %></th>								
								<th colspan="6" class="text-center"><%=Information.FlatCharge %></th>								
								 </tr>
 --%> 
							...
							<tbody>
								...
								if(request.getAttribute("EmpList") != null) {
									Iterator<AptModel> iterators = employeeList.iterator();
									while(iterators.hasNext()) 	{
										AptModel empDetails = iterators.next(); 
									%>
									<tr>
										<td tabindex="0" class="sorting_1"><%=empDetails.getRwaRegNo()%></td>
										<td><%=empDetails.getTowerNo()%></td>
										<td><%=empDetails.getSubscriptionType()%></td>
										<td><%=empDetails.getFlatFixCharge()%></td>

										<td><%=empDetails.getAmcCharge()%></td>
										<td><%=empDetails.getAuditCharge()%></td>
										<td><%=empDetails.getGymCharge()%></td>
										<td><%=empDetails.getSwmPoolCharge()%></td>
										<td><%=empDetails.getMiscCharge()%></td>
