...
//			var unit=" ";
			...
			$(document).ready(function(){
			    ...
			    $(document).on('click', "Button[name='xRmOpBqITMiD']", function(){ 
					var con = confirm("Do you really want to delete? ");
					if(con){
					   var xOpbalIDNO=$(this).val();
						var request =$.ajax({
							type:'POST',
							data:{Action:"RmITOPBiDxIfno",opbalIDNO : xOpbalIDNO },
						 	dataType: 'Json',
				...
				function sMITMOpBalList(result){
					...
					$('#itemMasterOpBalList').dataTable({
					    ...
					    "columns": [
							 ...
							 { title:	'Item Code & Name' ,data:"CodeAndName"},
							 { title:	'Mfg.Year'         ,data:"MfgYear"},
							 { title:	'Exp.Date'         ,data:"ExpDate"},
							 { title:	'Make'             ,data:"Make"},
 							 { title:	'Price'            ,data:"Price" },  
							 { title:	'Unit'             ,data:"Unit" },  
 							 { title:	'Open.Bal.Qty'    ,data:"OpBal" },  
							 { title:	'Action'          ,data:"iDNO",
						     	"render": function(data,type,row,meta){
						       	 	return	'<button type="button" name="xRmOpBqITMiD" align="middle" value="'+data+'" class="bg-warning text-dark" data-toggle="tooltip" data-placement="right" title="Click to Remove Book Information" ><span> &#9747; </span> </button>'; 
				        		},
					        }
						]
					}); 
