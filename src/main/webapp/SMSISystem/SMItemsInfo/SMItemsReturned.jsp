							...
							<ul class="nav nav-tabs card-header-tabs">
								<li class="nav-item " id="ViewUser"><a	class="nav-link active text-white "
									style="background-color: #000080" href="#"> <%=SMFixedValue.ITEM%>
										<%=SMFixedValue.RETURN%> <%=SMFixedValue.INFORMATION%> </a>
								</li>
								<li class="nav-item " id="XRiTBView"><a class="nav-link text-light" href="#"> 
									<%=SMFixedValue.LIST_ITEM_RETURN%></a>
								</li>
								<li class="nav-item " id="xRTodayiTRView"><a class="nav-link text-light" href="#"> 
									<%=SMFixedValue.TODAY_ITEM_RETURN%></a>
								</li>
			...
			<div id ="siIRifnoX" class=" col-sm-12 offset-sm-0 blinebl bg-light" Style="border: 1px solid">
				<br />
 				<div class="row ">
					...
					<div class="col-sm-2 offset-sm-0">
						<label for="rDueDate" class="h6"><%=SMFixedValue.DUEDATE%> </label>
					    <input type="text" class="form-control-plaintext blineTlw" id="rDueDate" name="rDueDate" disabled>
					</div>

					<div class="col-sm-2 offset-sm-0">
						<label for="itemQty" class="h6"><%=SMFixedValue.RETURN%> <%=SMFixedValue.QUANTITY%>
						</label> <input type="text" class="form-control   blineTl " id="itemQty"
							name="itemQty" required pattern="[0-9]{1,5}">
					</div>
					<div class="col-sm-2 offset-sm-0">
						<label for="xLateFee" class="h6"><%=SMFixedValue.LATEFEE%>
					...
					<div class="col-sm-1 offset-sm-0 ">
						<label for="xXLateFee" class="h6"></label>
						<button type="button" id="xNSMITBRetI" name="xNSMITBRetI"
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			function showAdmission(result){ 
 			...
 			$("#xNSMITBRetI").click(function() {
 				$("#xNSMITBRetI").attr("disabled", true)
				var request =$.ajax({
					type:'POST',
 	 				data:{Action:"xNSMItemBRInfo", SmitSnoid :$("#SmitSnoid").val(), itemQty: $("#itemQty").val(), xLateFee: $("#xLateFee").val() },
				 	dataType: 'Json',
					url:'../../SMItemReturnInfo',
					success:function(result){
			...
			$("#xRTodayiTRView").click(function(){ 
				if(xValidAdmissionNo($("#sAdmNo").val())){ 
					var request =$.ajax({
						type:'POST',
	 	 				data:{Action:"xRTodayiTRView", Student :  $("#sAdmNo").val()},
					 	dataType: 'Json',
						url:'../../SMItemReturnInfo',
						success:function(result){
			...
			function xRTodayiTRView(result){
				$('#xSMItemRList').dataTable({
				    ...
				    "columns": [
						 ...
						 { title:	'Due Date'        ,data:"IssToDate" }  ,  
						 { title:	'Issued Date'     ,data:"IssFromDate"} ,  
						 { title:	'Recieved By'     ,data:"TakenBy"}     ,
