...
public class DRVInfoUpdate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("Step 40.: Created sspinfo model successfully");
		...
		try {
			...
			user = DAODriverInfo.UpdateDRVInfo(user);
			if (user.isValid()) {
	...
	private DRVModel UpdateFormValueToUserObj(DRVModel user, HttpServletRequest request) {
		try {
	...
	private void removeSessionValue(HttpSession session) {
		...
		session.removeAttribute("inputLicenseNO");
		session.removeAttribute("inputLicenseCity");
		session.removeAttribute("inputLicenseDate");
		session.removeAttribute("inputExpiryDate");
		session.removeAttribute("DLIRTOAddress");
		session.removeAttribute("DLBlockNO");
		session.removeAttribute("DLSector");
		session.removeAttribute("DLCity");
		session.removeAttribute("DLState");
		session.removeAttribute("DLCountry");
		session.removeAttribute("DLZipCode");
